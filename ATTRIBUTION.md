...
The map plots iNaturalist research-grade observations precomputed into static viewport chunks, the same way the Falling Fruit records are chunked. The observations are sourced from a GBIF occurrence download of the iNaturalist Research-grade Observations dataset (GBIF.org, DOI 10.15468/dl.e6q6he), filtered to the app's taxa in the contiguous United States and thinned to roughly one point per location per species. GBIF mirrors only CC0, CC BY, and CC BY-NC records, so the compilation stays license-clean and noncommercial. Each point links back to its iNaturalist observation page. iNaturalist content remains owned by the contributor under Creative Commons Attribution-NonCommercial (CC BY-NC) unless otherwise specified. Do not reuse photos or other media from iNaturalist without checking the license on the individual observation. Coordinates that iNaturalist obscured at the observer's request are shown with an approximate-location badge, and observations obscured to protect sensitive taxa are omitted.
...
The Minerals map's locality points (`data/minerals-us.json`) are derived from the USGS Mineral Resources Data System (MRDS), 2022 release, across the **contiguous United States**. The national MRDS extract is filtered to craft-relevant commodities — pottery clay/kaolin; carving stone (soapstone/talc, gypsum/alabaster, serpentine, marble, dimension limestone, catlinite/pipestone); whetstone and knapping toolstone (novaculite, chert/flint, obsidian); and lapidary material (quartz, agate/chalcedony/jasper, petrified wood, gemstone/diamond) — recovered by commodity code plus free-text vetting, then thinned to a ~5 km grid and capped per material so no widespread material floods the map. MRDS is an economic-mining inventory: it has **no commodity code for novaculite or chert**, so those are recovered by free-text; treat positions and material categories as a **curated seed, not authoritative**, and note that positional accuracy varies (MRDS grades A–E) and the data is frozen at ~2011/2022. Catalog type-range descriptions draw on general geologic references (state geological survey commodity summaries); these are public-domain facts, not a licensed dataset.
...
Each record carries a land-manager classification (`perm`) used to show rock-collecting rules, assigned at build time by point-in-polygon against public-domain U.S. Government boundary layers: **National Park Service** unit boundaries (NPS Land Resources Division — a hard collecting ban, highest precedence), and **USDA Forest Service** administrative forest boundaries. Hardcoded factual special cases apply: catlinite/pipestone is treated as culturally restricted/tribal (quarrying at Pipestone National Monument is reserved by federal law, 36 CFR 7.42, to enrolled tribal members); the Crater of Diamonds State Park dig-and-keep rule is applied only to gemstone/diamond localities in the diatreme; and named commercial pay-to-dig quartz operations are downgraded to "Claimed / fee-dig". Every point with no positive boundary match defaults conservatively to **"Private / other" (verify)**. NOTE: national BLM Surface-Management-Agency polygons are not yet integrated (too large to fetch in the build), so BLM public land — where casual rockhounding is generally allowed under 43 CFR 8365.1-5 — currently shows the conservative "Private / other" default; adding the BLM SMA layer is the next refinement. **No ODbL/OpenStreetMap geometry is used or shipped.** The build script is `scripts/build_minerals_data.py`.

**Occurrence is never collecting permission.** Rock and mineral collecting is governed differently from foraging — the 36 CFR 2.1 personal-use exception for fruits/nuts/berries does **not** cover rocks or minerals. The per-record rule text is summarized from primary sources: the USFS Rockhounding Guide (FS-1091) and 36 CFR 228 Subpart C, the National Park Service prohibition (36 CFR 2.1), BLM rockhounding/mining-claim policy (43 CFR 8365.1-5), the Pipestone National Monument tribal-quarry rule (36 CFR 7.42), state trust/park variance, and Arkansas State Parks' Crater of Diamonds dig-and-keep policy. These are access-rule summaries, not a harvest-permission layer.

Sources: https://mrdata.usgs.gov/mrds/ · NPS unit boundaries (NPS Land Resources Division ArcGIS) · USFS Administrative Forest Boundaries (FSGeodata EDW) · https://www.fs.usda.gov/sites/default/files/rockhounding-guide-fseprd768373.pdf · https://www.ecfr.gov/current/title-36/section-2.1 · https://www.ecfr.gov/current/title-36/chapter-I/part-7/section-7.42 · https://www.ecfr.gov/current/title-43/section-8365.1-5 · https://www.arkansas.com/state-parks/explore/parks/crater-of-diamonds-state-park
