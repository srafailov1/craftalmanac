...
A local-first prototype for mapping edible plants and mushrooms in Virginia.
...
- Interactive Mapbox Outdoors basemap centered on Charlottesville when `config.js` has a Mapbox token, with movement constrained to Virginia and nearby border context.
...
- Automatic iNaturalist observation loading for the current map bounds, Virginia place scope, and selected food groups.
- Trimmed Virginia Falling Fruit import at `data/falling-fruit-virginia.json`.
...
Falling Fruit is not represented with starter records anymore. The browser app loads a trimmed Virginia JSON export from `data/falling-fruit-virginia.json`. Regenerate it from the downloaded Falling Fruit CSV archives with:

```sh
python3 scripts/build_falling_fruit_subset.py
```

The subset currently filters the full Falling Fruit archive to the app's food groups and the Virginia boundary stored in `data/virginia-boundary.json`.
...
- Add a city or county selector if the statewide Falling Fruit subset feels too broad.
