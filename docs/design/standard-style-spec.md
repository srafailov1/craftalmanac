...
| 7 | ~~`FALLING_FRUIT_AGGREGATE_LABEL_LAYER_ID`~~ | — | — | — | — | **REMOVED 2026-06-13.** Was permanently dead code (filter never matched any feature — see `KNOWN_ISSUES.md` #2, now resolved by removal); deleted as Phase 2 cleanup rather than ported. State-level context, if wanted, belongs to Phase 3's floating legend/season UI, not a map-painted label layer. |
